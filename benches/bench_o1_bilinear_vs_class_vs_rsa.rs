...
fn benchmark_o1_membership_ops(c: &mut Criterion) {
    ...
    let rsa_ep = rsa_acc_mem.add(&MEMBER_RAW);
    for v in [2u64, 3, 5, 7, 11] {
        rsa_acc_mem.add(&v);
    }
    let rsa_mem_proof = rsa_acc_mem.mem_proof_create(&rsa_ep);
    let rsa_blinded_mem = rsa_acc_mem.blind_mem_proof(&rsa_mem_proof);
    ...
    group.bench_function("rsa_blind_mem_proof", |b| {
        b.iter(|| {
            let _ = black_box(rsa_acc_mem.blind_mem_proof(&rsa_mem_proof));
        });
    });
    ...
    group.bench_function("rsa_unblind_mem_proof", |b| {
        b.iter(|| {
            let _ =
                black_box(rsa_acc_mem.unblind_mem_proof(&rsa_blinded_mem.0, &rsa_blinded_mem.1));
        });
    });

    group.bench_function("rsa_ver_blind_mem_proof_upd", |b| {
        b.iter(|| {
            black_box(rsa_acc_mem_upd.ver_blind_mem_proof_upd(
                &rsa_acc_t,
                &rsa_blinded_mem_for_ver,
                &rsa_upd_blinded_mem,
                &rsa_mem_aux,
    ...
    #[cfg(feature = "class-group")]
    {
        ...
        let class_ep = class_acc_mem.add(&MEMBER_RAW);
        for v in [2u64, 3, 5, 7, 11] {
            class_acc_mem.add(&v);
        }
        let class_mem_proof = class_acc_mem.mem_proof_create(&class_ep);
        let class_blinded_mem = class_acc_mem.blind_mem_proof(&class_mem_proof);
        ...
        group.bench_function("class_blind_mem_proof", |b| {
            b.iter(|| {
                let _ = black_box(class_acc_mem.blind_mem_proof(&class_mem_proof));
            });
        });
        ...
        group.bench_function("class_unblind_mem_proof", |b| {
            b.iter(|| {
                let _ = black_box(
                    class_acc_mem.unblind_mem_proof(&class_blinded_mem.0, &class_blinded_mem.1),
                );
            });
        });
        ...
        group.bench_function("class_ver_blind_mem_proof_upd", |b| {
            b.iter(|| {
                black_box(class_acc_mem_upd.ver_blind_mem_proof_upd(
                    &class_acc_t,
                    &class_blinded_mem_for_ver,
    ...
    bilinear_acc_mem.add(&member);
    for x in other_roots {
        bilinear_acc_mem.add(&x);
    }
    ...
    group.bench_function("bilinear_blind_mem_proof", |b| {
        b.iter(|| {
            ...
            let _ = black_box(
                bilinear_acc_mem
                    .blind_mem_proof(&mut rng, &bilinear_q, 1)
    ...
    bilinear_acc_mem_upd.add(&update);
    ...
    group.bench_function("bilinear_unblind_mem_proof", |b| {
        b.iter(|| {
            let _ = black_box(BilinearAccumulator::<Bls12_381>::unblind_mem_proof(
                &bilinear_pi_prime,
                &bilinear_r,
            ));
        });
    ...
    group.bench_function("bilinear_ver_blind_mem_proof_upd", |b| {
        b.iter(|| {
            black_box(bilinear_acc_mem_upd.ver_blind_mem_proof_upd(
                &bilinear_pi_blinded,
                &bilinear_pi_prime,
                &bilinear_acc_t,
                &bilinear_mem_delta,
...
fn benchmark_o1_non_membership_ops(c: &mut Criterion) {
    ...
    for v in [2u64, 3, 5, 7, 11] {
        rsa_acc_non_mem.add(&v);
    }
    let rsa_non_member = num_bigint::BigUint::from(NON_MEMBER_RAW);
    let rsa_blinded_non_mem = rsa_acc_non_mem.blind_non_mem_proof(&rsa_non_member);

    let mut rsa_acc_non_mem_upd = rsa_acc_non_mem.clone();
    rsa_acc_non_mem_upd.add(&UPDATE_RAW);
    ...
    let rsa_upd_blinded_non_mem =
        rsa_acc_non_mem_upd.blind_non_mem_proof_upd(&rsa_blinded_non_mem.0, &rsa_delta);

    group.bench_function("rsa_blind_non_mem_proof", |b| {
        b.iter(|| {
            let _ = black_box(rsa_acc_non_mem.blind_non_mem_proof(&rsa_non_member));
        });
    });
    ...
    group.bench_function("rsa_unblind_non_mem_proof", |b| {
        b.iter(|| {
            let _ = black_box(
                rsa_acc_non_mem_upd
                    .unblind_non_mem_proof(&rsa_blinded_non_mem.1, &rsa_upd_blinded_non_mem),
            );
        });
    });
    ...
    group.bench_function("rsa_ver_blind_non_mem_proof_upd", |b| {
        b.iter(|| {
            black_box(rsa_acc_non_mem_upd.ver_blind_non_mem_proof_upd(
                &rsa_acc_t_prime,
    ...
    #[cfg(feature = "class-group")]
    {
        ...
        for v in [2u64, 3, 5, 7, 11] {
            class_acc_non_mem.add(&v);
        }
        let class_non_member = class_acc_non_mem
            .group
            .hash_to_prime(&NON_MEMBER_RAW.to_be_bytes());
        let class_blinded_non_mem = class_acc_non_mem.blind_non_mem_proof(&class_non_member);

        let mut class_acc_non_mem_upd = class_acc_non_mem.clone();
        class_acc_non_mem_upd.add(&UPDATE_RAW);
        let class_acc_t_prime = class_acc_non_mem_upd.value().clone();
        let class_delta = class_acc_non_mem_upd.calculate_product_unreduced();
        let class_upd_blinded_non_mem =
            class_acc_non_mem_upd.blind_non_mem_proof_upd(&class_blinded_non_mem.0, &class_delta);

        group.bench_function("class_blind_non_mem_proof", |b| {
            b.iter(|| {
                let _ = black_box(class_acc_non_mem.blind_non_mem_proof(&class_non_member));
            });
        });
        ...
        group.bench_function("class_unblind_non_mem_proof", |b| {
            b.iter(|| {
                let _ =
                    black_box(class_acc_non_mem_upd.unblind_non_mem_proof(
        ...
        group.bench_function("class_ver_blind_non_mem_proof_upd", |b| {
            b.iter(|| {
                black_box(class_acc_non_mem_upd.ver_blind_non_mem_proof_upd(
                    &class_acc_t_prime,
        ...
        b.iter(|| {
            let _ = black_box(
                bilinear_acc_non_mem
                    .blind_non_mem_proof(&bilinear_non_mem_proof, bilinear_non_member),
            );
        });
    });
    ...
    group.bench_function("bilinear_unblind_non_mem_proof", |b| {
        b.iter(|| {
            let _ = black_box(bilinear_acc_non_mem_upd.unblind_non_mem_proof(
    ...
    group.bench_function("bilinear_ver_blind_non_mem_proof_upd", |b| {
        b.iter(|| {
            black_box(bilinear_acc_non_mem_upd.ver_blind_non_mem_proof_upd(
                &bilinear_acc_t,
